...
class Elasticsearch8SearchBackend(BaseSearchBackend):
    ...
    FIELD_MAPPINGS = {
        ...
        "long": {"type": "long"},
        "integer": {"type": "long"},
        "dense_vector": {"type": "dense_vector", "dims": 768},
    }
    ...
    def update(self, index, iterable, commit=True):
        ...
        for obj in iterable:
            try:
                ...
                final_data["_id"] = final_data[DJANGO_ID]
    ...
    def clear(self, models=None, commit=True):
        ...
        try:
            if models is None:
                ...
                query = {
                    "query": {
                        "query_string": {"query": " OR ".join(models_to_delete)}
                    }
                }
                generator = scan(
                    self.conn,
        ...
        **extra_kwargs,
        ...
        if facets is not None:
            ...
            for facet_fieldname, extra_options in facets.items():
                facet_field = index.get_facet_fieldname(facet_fieldname)
                facet_options = {
                    "meta": {"_type": "terms"},
        ...
        if vector_query is not None:
            ...
            if isinstance(kwargs["query"], dict) and "match_all" in kwargs["query"]:
                kwargs["knn"] = {
                    "field": vector_query.get(
                        "field", self.content_field_name + "_vector"
                    ),
                    "query_vector": vector_query["query_vector"],
                    "k": vector_query.get("k", 10),
                    "num_candidates": vector_query.get("num_candidates", 100),
                }
                if "boost" in vector_query:
                    kwargs["knn"]["boost"] = vector_query["boost"]
            else:
                script_query = {
                    "script_score": {
                ...
                if "boost" in vector_query:
                    script_query["script_score"]["boost"] = vector_query["boost"]
                kwargs["query"] = script_query
        ...
        if filters and "knn" not in kwargs:
    ...
    def more_like_this(
        ...
        **kwargs,
        ...
        try:
            ...
            self.log.exception(
                "Failed to fetch More Like This from Elasticsearch for "
                "document '%s'",
                doc_id,
            )
            raw_results = {}
    ...
    def _process_results(
        ...
        if "aggregations" in raw_results:
            ...
            facet_field_types = {}
            for field_name, field_class in unified_index.all_searchfields().items():
                if hasattr(field_class, "facet_for"):
                    continue
                facet_field_types[field_name] = field_class.field_type
            ...
            for facet_fieldname, facet_info in raw_results["aggregations"].items():
                ...
                if facet_type == "terms":
                    ...
                    original_field = facet_fieldname
                    if original_field in facet_field_types:
                        field_type = facet_field_types[original_field]
                    else:
                        for candidate_field, candidate_type in facet_field_types.items():
                            if facet_fieldname.endswith(
                                "_" + candidate_field
                            ) or facet_fieldname == candidate_field:
                    ...
                    for individual in facet_info["buckets"]:
                        ...
                        if field_type in numeric_types:
                            if field_type == "boolean":
                                key = key is True or key == "true" or key == True
                            elif field_type in ("float", "long", "integer"):
                                try:
                                    key = float(key)
                                    if key == int(key):
                                        key = int(key)
                                    ...
                                    pass
                        raw_buckets.append((key, individual["doc_count"]))

                    facets["fields"][facet_fieldname] = raw_buckets
        ...
        for raw_result in raw_results.get("hits", {}).get("hits", []):
            ...
            if model and model in indexed_models:
                ...
                for key, value in source.items():
                    ...
                    if index and string_key in index.fields and hasattr(
