...
class Elasticsearch8SearchBackend(BaseSearchBackend):
    ...
    def update(self, index, iterable, commit=True):
        ...
        if prepped_docs:
            ...
            bulk(self.conn, actions=actions)
    ...
    def build_schema(self, fields):
        ...
        for _, field_class in fields.items():
            ...
            if field_class.field_type == "vector":
                dims = getattr(field_class, "dims", 384)
                similarity = getattr(field_class, "similarity", "cosine")
                field_mapping["dims"] = dims
                field_mapping["similarity"] = similarity
    ...
    def _process_results(
        ...
        if "aggregations" in raw_results:
            ...
            for facet_fieldname, facet_info in raw_results["aggregations"].items():
                facet_type = facet_info.get("meta", {}).get("_type")
                if facet_type == "terms":
    ...
    def _get_facet_fieldname(self, fieldname):
        return fieldname
    ...
    def vector_search(self, field_name, vector, k=10, **kwargs):
        if not self.setup_complete:
            self.setup()
        ...
        if kwargs.get("models"):
            model_choices = sorted(get_model_ct(m) for m in kwargs["models"])
            if model_choices:
                filters.append({"terms": {DJANGO_CT: model_choices}})

        if filters:
            query["knn"]["filter"] = {"bool": {"must": filters}} if len(filters) > 1 else filters[0]

        try:
            raw_results = self.conn.search(
                body={"query": {"match_all": {}}, "knn": query["knn"]},
                index=self.index_name,
            )
        except elasticsearch.TransportError:
            if not self.silently_fail:
                raise
            self.log.exception("Failed to perform vector search on field '%s'", field_name)
            raw_results = {}
